...
const Elephant = () => {
  const [selectedSound, setSelectedSound] = useState("/audio/bark.mp3"); // Default to dog sound
  ...
  useEffect(() => {
    ...
    camera.position.set(0.2, 2, 0.75);
    ...
    let dogModel;
    const loader = new GLTFLoader().setPath("/elephant/");
    loader.load(
      "elephant.gltf",
      (gltf) => {
        dogModel = gltf.scene;
    ...
    const animate = () => {
      ...
      if (dogModel) {
        ...
        if (isJumping) {
          if (jumpDirection === 1) {
            dogModel.position.y += 0.007;
            if (dogModel.position.y >= 1.05 + 0.25) jumpDirection = 0;
          } else {
            dogModel.position.y -= 0.007;
      ...
      <div ref={mountRef}>
        <div className={styles.ButtonRight}>
          <div id="soundSelect">
            ...
            >
              <option value="/audio/bird.mp3">Elephant Sound</option>
