...
const Tiger = () => {
  const [selectedSound, setSelectedSound] = useState("/audio/roar.mp3"); // Default to tiger sound
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        tigerModel.position.set(0, 1.05, 0);
        scene.add(tigerModel);
    ...
    const animate = () => {
      ...
      if (tigerModel) {
        if (isTurning) {
          const rotationStep = 0.04 * turnDirection;
          tigerModel.rotation.y += rotationStep;
          turnAmount += Math.abs(rotationStep);
          if (turnAmount >= 2 * Math.PI) {
            isTurning = false;
            tigerModel.rotation.y = 0;
          }
        }
        if (isJumping) {
          if (jumpDirection === 1) {
            tigerModel.position.y += 0.007;
            if (tigerModel.position.y >= 1.05 + 0.25) jumpDirection = 0;
          } else {
            tigerModel.position.y -= 0.007;
            if (tigerModel.position.y <= 1.05) {
              tigerModel.position.y = 1.05;
              isJumping = false;
            }
          }
        }
        ...
        <div className={styles.ButtonRight}>
          <div id="soundSelect">
            <select
              ...
              value={selectedSound || "/audio/roar.mp3"}
            >
